...
if(DEVELOPMENT) {
	...
	function getScavengerNumber() {
		for (var e, r = maxPlayers; 11 > r; ++r)
		if (enumStruct(r).length > 0) {
			e = r;
			break
		}
		return e
	...
	function checkIfSeaMap() {
		...
		for (var r = 0; r < maxPlayers; ++r)
		if (!propulsionCanReach("wheeled01", startPositions[me].x, startPositions[me].y, startPositions[r].x, startPositions[r].y)) {
			for (var t = 0, a = 0; a < maxPlayers; ++a) propulsionCanReach("hover01", startPositions[r].x, startPositions[r].y, startPositions[a].x, startPositions[a].y) || (t += 1);
			if (t !== maxPlayers - 1) {
				e = !0;
				break
			}
		...
		if (e === !0)
		for (var r = 0; r < maxPlayers; ++r) {
			if (propulsionCanReach("wheeled01", startPositions[me].x, startPositions[me].y, startPositions[r].x, startPositions[r].y) && r !== me && !allianceExistsBetween(r, me) && enumDroid(r).length > 0) {
				seaMapWithLandEnemy = !0;
				break
			}
			if (seaMapWithLandEnemy === !0) break
		}
		return e
	}

	function freeForAll() {
		for (var e = !0, r = 0; r < maxPlayers; ++r)
		if (r != me && !allianceExistsBetween(r, me)) {
			var t = countStruct("A0LightFactory", r) + countStruct("A0CyborgFactory", r),
			a = countDroid(DROID_ANY, r);
			if (a > 0 || t > 0) {
				e = !1;
				break
			}
	...
	function CheckStartingBases() {
		...
		for (var e = subpersonalities[personality].primaryWeapon.weapons.length, r = 0; e > r; ++r)
		if (isDesignable(subpersonalities[personality].primaryWeapon.weapons[r].stat)) return !0;
		return !1
	}

	function countAllResources() {
		for (var e = enumFeature(-1, oilResources), r = 0; r < maxPlayers; ++r)
		for (var t = enumStruct(r, structures.derricks), a = 0; a < t.length; ++a) e.push(t[a]);
		if (isDefined(getScavengerNumber()))
		for (var t = enumStruct(getScavengerNumber(), structures.derricks), a = 0; a < t.length; ++a) e.push(t[a]);
		return e.length
	}

	function mapOilLevel() {
		var e = countAllResources() / maxPlayers,
		r = "";
		return r = 8 >= e ? "LOW" : e > 8 && 16 >= e ? "MEDIUM" : "HIGH"
	}
}
